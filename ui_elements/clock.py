...
class Clock(object):
    def __init__(self, root, w, h, init_h, init_m):
        # Smaller canvas
        self.canvas = tk.Canvas(root, width=150, height=100, bg="#f0f0f0", highlightthickness=0)
        self.canvas.place(x=math.floor(0.75 * w), y=30)

        # Title label
        tk.Label(self.canvas, text="Current time", font=("Arial", 12), bg="#f0f0f0").place(relx=0.5, y=10, anchor="n")

        # Digital time label (smaller)
        self.label = tk.Label(self.canvas, text="", font=("Arial", 20, "bold"), bg="#f0f0f0", fg="midnightblue")
